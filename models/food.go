...
func GetFoodsByName(name string) ([]*Food, error) {
	foods := make([]*Food, 0)
	if result := VDB.Preload("FoodGroup").Limit(10).Where("Name LIKE ?", name+"%").Find(&foods); result.Error != nil {
		return nil, result.Error
	}
	if len(foods) == 0 {
		if result := VDB.Preload("FoodGroup").Limit(10).Where("Name LIKE ?", "%"+name+"%").Find(&foods); result.Error != nil {
...
func GetFoodsByNameGr(nameGr string) ([]*Food, error) {
	foods := make([]*Food, 0)
	if result := VDB.Preload("FoodGroup").Limit(10).Where("NameGR LIKE ?", nameGr+"%").Find(&foods); result.Error != nil {
		return nil, result.Error
	}
	if len(foods) == 0 {
		if result := VDB.Preload("FoodGroup").Limit(10).Where("NameGR LIKE ?", "%"+nameGr+"%").Find(&foods); result.Error != nil {
